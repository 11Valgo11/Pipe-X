...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/21 22:43:13 by ayel-bou          #+#    #+#             */
/*   Updated: 2025/03/04 22:13:27 by ayel-bou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*findpath(char **args, char **env, t_exec *exec)
{
	...
	if (exec->path)
	{
		...
		while (exec->path[i])
		{
			exec->partjoin = ft_strjoin(exec->path[i], "/", NULL);
			exec->cmdjoined = ft_strjoin(exec->partjoin, args[0], NULL);
			free(exec->partjoin);
